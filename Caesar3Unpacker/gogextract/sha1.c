...
static void sha1_transform(uint32_t state[5], const uint8_t buffer[64])
{
    uint32_t a, b, c, d, e;
    uint32_t block[16];
    /* Convert input buffer to 16 big-endian 32-bit integers */
    for (int i = 0; i < 16; i++) {
        uint8_t b0 = buffer[4*i+0];
        uint8_t b1 = buffer[4*i+1];
        uint8_t b2 = buffer[4*i+2];
        uint8_t b3 = buffer[4*i+3];
        block[i] = (int32_t) ((b0 << 24) | (b1 << 16) | (b2 << 8) | b3);
    }
...
static int sha1_read_bytes(void *source_data, void *buffer, int max_size)
{
    ...
    sha1_update(&data->sha, buffer, read_bytes);
    return read_bytes;
}

static int sha1_seek_to(void *source_data, long offset, int whence)
{
    io_source *base = ((sha1_io_data *) source_data)->base;
    return base->seek_to(base->data, offset, whence);
}

static int sha1_has_error(void *source_data)
{
    io_source *base = ((sha1_io_data *) source_data)->base;
    return base->has_error(base->data);
}

static long sha1_get_pos(void *source_data)
{
    io_source *base = ((sha1_io_data *) source_data)->base;
